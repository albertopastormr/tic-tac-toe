...
class NeuralNet:
    ...
    def __init__(self, num_features, num_hidden_layers, num_hidden_nodes, num_labels, th=None,
        ...
        self.th = self._initial_thetas() if th is None else np.asarray(th)
    ...
    def _cost(self, y_true, y_pred):
        ...
        for t in self.th[:-1]:
            J += (self.reg / (2 * m)) * (np.sum(t[:][1:] ** 2))
        J += np.sum(self.th[-1][:][1:] ** 2)
        return J
    ...
    def _backprop(self, X, y):
        ...
        for i in range(1, len(self.layers_sizes)):
            #print(f'i={i}, a[0].shape{a[0].shape}, th[0].shape{self.th[0].shape}')
            z.append( np.matmul( a[i-1], self.th[i-1].T))
            a.append( np.insert(self.fun(z[i]), 0, values=np.ones(m), axis=1))
        pred = self.fun(z[-1])
        ...
        for i in range(0, len(th_)):
            if i + 1 == len(th_):
                ...
                delta_slice_index = 1
            D.append((delta_[i][delta_slice_index:,:]/m + (th_[i]*self.reg)/m).ravel())
    ...
    def fit(self, X, y, maxiter=70):
        ...
        """
        fmin = opt.minimize(fun=self._backprop, x0=np.append(self.th[0], self.th[1]).reshape(-1), args=(X, y),
    ...
    def predict(self, X, threshold=0.5):
        ...
        for x in X:
            ...
            for i in range(1, len(self.layers_sizes)):
                z.append( np.matmul( a[i-1], self.th[i-1].T))
                a.append( np.insert(self.fun(z[i]), 0, values=np.ones(1), axis=1))
